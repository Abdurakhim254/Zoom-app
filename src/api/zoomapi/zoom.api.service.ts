...
export class ZoomApiService {
  ...
  async deleteMeeting(meetingId: number) {
    const [deletedmeeting, result] = await Promise.all([
      this.meetingservice.deleteMeetingById(meetingId),
      this.zoomService.remove(meetingId),
    ]);
  ...
  async getMeetingById(meetingId: number) {
    const [meeting, result] = await Promise.all([
      this.meetingservice.getMeetingById(meetingId),
      this.zoomService.findOneByMeetingId(meetingId),
    ]);
  ...
  async updateMeeting(body: UpdateZoomMeetingDto, meetingId: number) {
    const [meeting, result] = await Promise.all([
      this.meetingservice.updateMeetingById(meetingId, body),
      this.zoomService.update(meetingId, body),
    ]);
